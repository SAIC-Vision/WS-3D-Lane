...
class LaneEval(object):
    def __init__(self, config):
        self.dataset_dir = config.dataset.val.data_rt[0]
        self.K = np.array(config.K)
        self.no_centerline = config.no_centerline
        ...
        self.x_min = np.array(config.top_view_region)[0, 0]
        self.x_max = np.array(config.top_view_region)[1, 0]
        self.y_min = np.array(config.top_view_region)[2, 1]
        self.y_max = np.array(config.top_view_region)[0, 1]
        ...
        if match_results.shape[0] > 0:
            for i in range(len(match_results)):
                if match_results[i, 2] < self.dist_th * self.y_samples.shape[0]:
                    ...
                    # if x_dist_mat_close[gt_i, pred_i] > 0.3:
                    #     print(raw_file, x_dist_mat_close[gt_i, pred_i])
