...
def define_args():
    ...
    parser.add_argument("--pred_cam", type=str2bool, nargs='?', const=True, default=True, help="use network to predict camera online?")
...
class Visualizer:
    def __init__(self, args, vis_folder='val_vis'):
        ...
        savedir = os.path.join('./example', vis_folder)
        os.makedirs(savedir, exist_ok=True)
        self.no_3d = args.no_3d
        self.no_centerline = args.no_centerline
        self.vgg_mean = args.input_mean
        self.vgg_std = args.input_std
        ...
        x_min = np.array(args.top_view_region)[0, 0]
        x_max = np.array(args.top_view_region)[1, 0]
...
def homography_im2ipm_norm(top_view_region, org_img_size, crop_y, resize_img_size, cam_pitch, cam_height, K, cam_yaw = 0, cam_roll = 0):
    """
    ...
    H_g2im = homograpthy_g2im(cam_pitch, cam_height, K, gt_cam_yaw=cam_yaw, gt_cam_roll = cam_roll)
    #print(H_g2im)
...
def homograpthy_g2im(cam_pitch, cam_height, K, gt_cam_yaw = 0, gt_cam_roll = 0):
    # transform top-view region to original image region
    gt_cam_yaw = - gt_cam_yaw
    R_yaw = np.array([[np.cos(gt_cam_yaw), -np.sin(gt_cam_yaw), 0],
                    [np.sin(gt_cam_yaw), np.cos(gt_cam_yaw), 0],
                    [0, 0, 1]])
    ...
    #print(T)
    #H_g2im = np.matmul(K, np.concatenate([R_g2c[:, 0:2], [[0], [cam_height], [0]]], 1))
    H_g2im = np.matmul(K, np.concatenate([R_g2c[:, 0:2], T], 1))
