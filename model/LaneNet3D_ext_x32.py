...
'''
On the base of 3D-LaneNet, WS-3Dlane add the branch of segmetation branch and expand anchors
'''
...
class LanePredictionHead(nn.Module):
    def __init__(self, num_lane_type, num_y_steps, batch_norm=False):
        ...
        layers = []
        layers += make_one_layer(512, 128, kernel_size=3, padding=(0, 1), batch_norm=batch_norm)
        layers += make_one_layer(128, 128, kernel_size=3, padding=(0, 1), batch_norm=batch_norm)
        layers += make_one_layer(128, 128, kernel_size=3, padding=(0, 1), batch_norm=batch_norm)

        layers += make_one_layer(128, 128, kernel_size=5, padding=(0, 2), batch_norm=batch_norm)
        layers += make_one_layer(128, 128, kernel_size=5, padding=(0, 2), batch_norm=batch_norm)
        layers += make_one_layer(128, 128, kernel_size=5, padding=(0, 2), batch_norm=batch_norm)
        layers += make_one_layer(128, 128, kernel_size=5, padding=(0, 2), batch_norm=batch_norm)
        ...
        dim_rt_layers = []
        dim_rt_layers += make_one_layer(256, 256, kernel_size=(5, 1), padding=(2, 0), batch_norm=batch_norm)
        dim_rt_layers += [nn.Conv2d(256, self.num_lane_type*self.anchor_dim, kernel_size=(5, 1), padding=(2, 0))]
        self.dim_rt = nn.Sequential(*dim_rt_layers)

    def forward(self, x):
        x = self.features(x)
        # x suppose to be N X 64 X 4 X ipm_w/8, reshape to N X 128 X ipm_w/4 X 1
        sizes = x.shape
        x = x.reshape(sizes[0], sizes[1]*sizes[2]//2, sizes[3]*2, 1)
        x = self.dim_rt(x)
        x = x.squeeze(-1).transpose(1, 2)
        # apply sigmoid to the probability  and visibility terms to make it in (0, 1)

        x[:, :, 2*self.num_y_steps: 3*self.num_y_steps] = \
            torch.sigmoid(x[:, :, 2*self.num_y_steps: 3*self.num_y_steps])
        x[:, :, -1] = \
            torch.sigmoid(x[:, :, -1])
        return x

def make_upsampler_layer(in_channels, out_channels, kernel_size=3, padding=1, stride=1, output_padding=1, batch_norm=False):
    conv2d = nn.ConvTranspose2d(in_channels, out_channels, kernel_size=kernel_size, padding=padding, stride=stride, output_padding = output_padding)
    if batch_norm:
        layers = [conv2d, nn.BatchNorm2d(out_channels), nn.ReLU(inplace=True)]
    else:
        layers = [conv2d, nn.ReLU(inplace=True)]
...
class TopViewDecoder(nn.Module):
    def __init__(self, num_classes, batch_norm=False):
        super().__init__()


        self.layers = []
        self.layers += make_upsampler_layer(512, 128, kernel_size=3, stride=2, padding=1, output_padding=1, batch_norm=batch_norm)
        self.layers += make_one_layer(128, 128, kernel_size=3, padding=1, batch_norm=batch_norm)
        self.layers += make_one_layer(128, 128, kernel_size=3, padding=1, batch_norm=batch_norm)

        self.layers += make_upsampler_layer(128, 32, kernel_size=3, stride=2, padding=1, output_padding=1, batch_norm=batch_norm)
        self.layers += make_one_layer(32, 32, kernel_size=3, padding=1, batch_norm=batch_norm)
        self.layers += make_one_layer(32, 32, kernel_size=3, padding=1, batch_norm=batch_norm)

        self.features = nn.Sequential(*self.layers)

        self.output_conv = nn.ConvTranspose2d(32, num_classes, kernel_size=3, stride=2, padding=1, output_padding=1, bias=True)
...
class Net(nn.Module):
    def __init__(self, config, args, debug=False):
        super().__init__()
        ...
        self.num_y_steps = config.num_y_steps
        if config.no_3d:
            self.anchor_dim = config.num_y_steps + 1
        else:
            self.anchor_dim = 3*config.num_y_steps + 1 + 12

        # define required transformation matrices
        # define homographic transformation between image and ipm
        org_img_size = np.array([config.org_h, config.org_w])
        resize_img_size = np.array([config.resize_h, config.resize_w])
        cam_pitch = np.pi / 180 * config.pitch

        self.cam_height = torch.tensor(config.cam_height).unsqueeze_(0).expand([self.batch_size, 1]).type(torch.FloatTensor)
        self.cam_pitch = torch.tensor(cam_pitch).unsqueeze_(0).expand([self.batch_size, 1]).type(torch.FloatTensor)
        self.cam_height_default = torch.tensor(config.cam_height).unsqueeze_(0).expand(self.batch_size).type(torch.FloatTensor)
        self.cam_pitch_default = torch.tensor(cam_pitch).unsqueeze_(0).expand(self.batch_size).type(torch.FloatTensor)

        # image scale matrix
        self.S_im = torch.from_numpy(np.array([[config.resize_w,              0, 0],
                                               [            0,  config.resize_h, 0],
                                               [            0,              0, 1]], dtype=np.float32))
        self.S_im_inv = torch.from_numpy(np.array([[1/np.float(config.resize_w),                         0, 0],
                                                   [                        0, 1/np.float(config.resize_h), 0],
                                                   [                        0,                         0, 1]], dtype=np.float32))
        self.S_im_inv_batch = self.S_im_inv.unsqueeze_(0).expand([self.batch_size, 3, 3]).type(torch.FloatTensor)

        # image transform matrix
        H_c = homography_crop_resize(org_img_size, config.crop_y, resize_img_size)
        self.H_c = torch.from_numpy(H_c).unsqueeze_(0).expand([self.batch_size, 3, 3]).type(torch.FloatTensor)

        # camera intrinsic matrix
        self.K = torch.from_numpy(np.array(config.K)).unsqueeze_(0).expand([self.batch_size, 3, 3]).type(torch.FloatTensor)
        ...
        H_g2cam = np.array([[1,                             0,               0],
                            [0, np.sin(-cam_pitch), config.cam_height],
        ...
        H_ipmnorm2g = homography_ipmnorm2g(config.top_view_region)
        ...
        if not self.no_cuda:
            self.M_inv = self.M_inv.to(self.device)
            self.S_im = self.S_im.to(self.device)
            self.S_im_inv = self.S_im_inv.to(self.device)
            self.S_im_inv_batch = self.S_im_inv_batch.to(self.device)
            self.H_c = self.H_c.to(self.device)
            self.K = self.K.to(self.device)
            self.H_g2cam = self.H_g2cam.to(self.device)
            self.H_ipmnorm2g = self.H_ipmnorm2g.to(self.device)
            self.cam_height_default = self.cam_height_default.to(self.device)
            self.cam_pitch_default = self.cam_pitch_default.to(self.device)
            self.cam_pitch = self.cam_pitch.to(self.device)

        # Define network
        self.im_encoder = VggEncoder(batch_norm=config.batch_norm)

        if self.pred_cam:
            self.road_plane_pred_head = RoadPlanePredHead(config.resize_h, config.resize_w, batch_norm=False)
        ...
        size_top1 = torch.Size([self.batch_size, config.ipm_h, config.ipm_w])
        self.project_layer1 = ProjectiveGridGenerator(size_top1, self.M_inv, config.no_cuda)
        size_top2 = torch.Size([self.batch_size, np.int(config.ipm_h / 2), np.int(config.ipm_w / 2)])
        self.project_layer2 = ProjectiveGridGenerator(size_top2, self.M_inv, config.no_cuda)
        size_top3 = torch.Size([self.batch_size, np.int(config.ipm_h / 4), np.int(config.ipm_w / 4)])
        self.project_layer3 = ProjectiveGridGenerator(size_top3, self.M_inv, config.no_cuda)
        size_top4 = torch.Size([self.batch_size, np.int(config.ipm_h / 8), np.int(config.ipm_w / 8)])
        self.project_layer4 = ProjectiveGridGenerator(size_top4, self.M_inv, config.no_cuda)

        self.dim_rt1 = nn.Sequential(*make_one_layer(256, 128, kernel_size=1, padding=0, batch_norm=config.batch_norm))
        self.dim_rt2 = nn.Sequential(*make_one_layer(512, 256, kernel_size=1, padding=0, batch_norm=config.batch_norm))
        self.dim_rt3 = nn.Sequential(*make_one_layer(512, 256, kernel_size=1, padding=0, batch_norm=config.batch_norm))

        self.top_pathway = TopViewPathway(config.batch_norm)
        self.lane_out = LanePredictionHead(self.num_lane_type, self.num_y_steps, config.batch_norm)
        self.top_decode = TopViewDecoder(num_classes=2, batch_norm=config.batch_norm)

    def forward(self, input, cam_pitch_gt=None):
        ...
        if self.pred_cam:
            ...
            cam_pitch = pred_cam
            cam_pitch = torch.sigmoid(cam_pitch) * torch.pi / 18 - torch.pi / 36  # limit the range to [-pi/36, pi/36]
            ...
            with torch.no_grad():
                self.H_g2cam[:, 1, 1] = torch.sin(-cam_pitch).squeeze()
                self.H_g2cam[:, 2, 1] = torch.cos(-cam_pitch).squeeze()
                self.H_g2cam[:, 1, 2] = cam_height.squeeze()
                M_ipm2im = torch.bmm(self.H_g2cam, self.H_ipmnorm2g)
                M_ipm2im = torch.bmm(self.K, M_ipm2im)
                M_ipm2im = torch.bmm(self.H_c, M_ipm2im)
                M_ipm2im = torch.bmm(self.S_im_inv_batch, M_ipm2im)
        ...
        out = self.lane_out(x)
        out_seg = self.top_decode(x)

        if self.debug:
            return out, cam_height, cam_pitch, x1, x2, x3, x4, x1_proj, x2_proj_out, x3_proj_out, x4_proj_out, top_2, top_3, top_4

        return out, cam_height, cam_pitch, out_seg

    def update_projection(self, config, cam_height, cam_pitch):
        """
        ...
        for i in range(self.batch_size):
            M, M_inv = homography_im2ipm_norm(np.array(config.top_view_region), np.array([config.org_h, config.org_w]),
                                              config.crop_y, np.array([config.resize_h, config.resize_w]),
                                              cam_pitch[i].data.cpu().numpy(), cam_height[i].data.cpu().numpy(), np.array(config.K))
